...
def slice(source,target,A4=A4,resize=resize,width_line=line_width,colorBG="black",color1="white",color2="yellow"):
    '''
    ...
    for x in range(gw):
        for y in range(gh):
            ...
            if(colorBG == "black"):
                ...
                if(np.max(subimg) < 255): # Sauvegarder seulement si l'image nest pas vide
                    Image.fromarray(subimg).save(target+str(y)+"_"+str(x)+".png")
            else:
